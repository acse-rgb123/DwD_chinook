...
class SubschemaPipeline:
    def __init__(self, db_file, embedding_handler):
        ...
        self.schema_mapper = SchemaMapper(self.schema, self.foreign_keys, self.embedding_handler)
        self.subschema_creator = SubschemaCreator(self.schema, self.foreign_keys)

    def extract_keywords(self, user_query):
        """Extract keywords using RoBERTa model."""
    ...
    def identify_relevant_tables(self, keywords, similarity_threshold=0.45):
        ...
        relevant_tables = self.schema_mapper.identify_relevant_tables(keywords, similarity_threshold)
    ...
    def create_joins(self, relevant_tables):
        ...
        formatted_joins = []
        for join in joins:
            # Ensure join is returned as a dictionary with the expected structure
            formatted_joins.append({
                'tables': (join[0], join[1]),
                'columns': (join[2]['from_column'], join[2]['to_column'])
            })
            print(f"{join[0]} <-> {join[1]} (From: {join[2]['from_column']}, To: {join[2]['to_column']})")
