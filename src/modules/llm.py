...
class LLM:
    ...
    def generate_sql_with_rag(self, user_query, joins, relevant_docs):
        ...
        response = openai.ChatCompletion.create(
            model="gpt-4",
            messages=[
                {"role": "system", "content": "You are a master at generating SQL queries based on user inputs and database schema."},
    ...
    def analyze_result(self, user_query, sql_query, result_df):
        ...
        response = openai.ChatCompletion.create(
            model="gpt-4",
            messages=[
                {"role": "system", "content": "You analyze SQL queries and their results to provide accurate answers to user questions."},
                {"role": "user", "content": input_text}
            ],
            max_tokens=200,
            temperature=0.2
