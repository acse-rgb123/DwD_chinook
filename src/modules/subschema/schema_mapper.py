...
class SchemaMapper:
    ...
    def map_keywords_to_columns(self, keywords, relevant_tables, similarity_threshold=0.40):
        mapped_columns = {}
        similarity_scores = []
        keyword_embeddings = self.embedding_handler.get_embeddings_batch(keywords)
        ...
        similarity_df = pd.DataFrame(similarity_scores, columns=['Keyword', 'Table.Column', 'Similarity'])
